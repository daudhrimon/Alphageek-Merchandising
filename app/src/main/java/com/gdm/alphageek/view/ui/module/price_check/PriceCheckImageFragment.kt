...
import com.gdm.alphageek.databinding.FragmentPriceCheckImageBinding
...
@AndroidEntryPoint
class PriceCheckImageFragment : Fragment() {
    private lateinit var binding:FragmentPriceCheckImageBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentPriceCheckImageBinding.inflate(layoutInflater)
        ...
        binding.shareBtn.setOnClickListener {
            if (imageList.isNotEmpty()) {
                ...
                showErrorToast(requireContext(),"Please select image first")
            }
        }
        ...
        visitViewModel.insertVisitResponse.observe(viewLifecycleOwner) {
            if (it != null) {
                if (it.toInt() != -1) {
                    showSuccessToast(requireContext(),"Price Checking visit successfully Added")
                    findNavController().navigate(R.id.dashboardFragment)
                } else {
                    showErrorToast(requireContext(),"Failed to visit Price Check")
                }
            }
        }
